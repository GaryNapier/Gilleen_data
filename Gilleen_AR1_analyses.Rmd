...
for (i in seq(Analyses_nos)){
  if (is.even(i)){
  Model_names[i] <- sprintf("A%g_beads", Analyses_nos[i])  
  }else{
  Model_names[i] <- sprintf("A%g_PDG", Analyses_nos[i])  
  }
}
...
for (i in 1:length(Analyses_nos)){
  ...
  grid.arrange(Om2_box+theme(legend.position="none"),
               ...
               top = sprintf("Om2, Om3, log(Nu), M2, M3, Phi2 & Phi3 posteriors; A %d. %s", Analyses_nos[i], Datasets[i]), 
...
for (i in 1:length(Analyses_nos)){
  ...
  grid.arrange(Om2_dens+theme(legend.position="none"),
               ...
               top = sprintf("Om2, Om3, log(Nu), M2, M3, Phi2 & Phi3 posteriors; A %d. %s", 
                             Analyses_nos[i], Datasets[i]), 
               My_legend,
               heights=rep(5, 2), widths=rep(3, 4))
